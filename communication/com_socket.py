        ...
        if self.connected:
            try:
                ...
                while len(self.data) < msg_size:
                    received_data = self.s.recv(msg_size-len(self.data))
...
class DataHolder:
    def __init__(self):
        self.frame_stack = queue.Queue()
        self.discrete_stack = queue.Queue()

    def get_frame(self):
        if not self.frame_stack.empty():
            return self.frame_stack.get_nowait()

    def get_discrete(self):
        if not self.discrete_stack.empty():
            return self.discrete_stack.get_nowait()

    def set_frame(self, frame):
        if frame is None:
            return
        if self.frame_stack.full():
            print("Data Holder: Frame stack is full!")
            return
        self.frame_stack.put_nowait(frame)

    def set_discrete(self, discrete):
        if discrete is None:
            return
        if self.discrete_stack.full():
            print("Data Holder: Discrete stack is full!")
            return
        self.discrete_stack.put_nowait(discrete)
